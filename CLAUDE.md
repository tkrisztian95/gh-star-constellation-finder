...
- **Release-sync rule: `package.json` version + `CHANGELOG.md` entry MUST land before any `v*` tag is pushed.** Whenever a new `v*` tag is about to be cut (whether you're suggesting it to the user or executing the tag yourself), first land a commit on `main` that:
  - Bumps the `version` field in [package.json](./package.json) to the target version (without a leading `v`).
  ...
  Reference: the [v0.1.0](./CHANGELOG.md) section is the canonical format example. If the version is a patch (e.g. `v0.1.1`) and ships only one fix, a single-bullet section is fine — but both the `package.json` bump and the section MUST exist before the tag is pushed. Push the tag first and the GitHub release auto-notes will be the only record of what shipped; the curated changelog never catches up later, and any code that reads `package.json` (e.g. `--version`) will report the wrong number. Never suggest `git tag vX.Y.Z` to the user without confirming the `package.json` bump + changelog entry already exist or are bundled in the same suggestion.

  **This is enforced.** The [`.husky/pre-push`](./.husky/pre-push) hook refuses to push any `v*` tag whose tagged commit doesn't have a matching `package.json` version and a `## [<version>]` heading in `CHANGELOG.md`. If you ever need to bypass for a genuine emergency, `git push --no-verify` is the escape hatch — but the right reflex is to fix the gap on `main` and retry, not bypass.
...
- **File logging is always-on.** `src/logger.ts` writes JSONL to `$XDG_STATE_HOME/gh-star-constellation-finder/app.log` (override with `LOG_FILE`; relative paths resolve against `process.cwd()` so `LOG_FILE=app.log` writes to the project root). Level via `LOG_LEVEL` (default `info`). For any operational log in non-test source, use `logger.{debug,info,warn,error}` — never `console.*` (the `no-console` ESLint rule will fail the lint step otherwise, and stray `console.*` writes corrupt Ink TUI rendering). Existing instrumentation already covers app lifecycle, phase boundaries, user decisions (confirm/scope/strategy/interrupt/save), and failure paths — when adding new code, prefer extending the existing `logger.info` site at the relevant phase boundary over creating a new one. Reserve `logger.debug` for per-item traces (per-repo, per-mutation) that would flood the log at `info`. AI call tracing belongs in Langfuse, not here — don't duplicate.
