# provider-embeddings Specification

## Purpose
The `embed()` seam on the `AIProvider` abstraction: batch text→vector with per-backend model/dimension selection and a stable `embedderId` identity for cache-staleness detection. Created by archiving change analysis-embeddings.
